...
namespace ScottEwing.Trajectory{
    public class PhysicsTrajectoryBehaviour : MonoBehaviour{
        ...
        public bool TryCalculateLaunchData(Vector3 targetPosition, out LaunchData data, Vector3 startPosition) {
            var displacementY = targetPosition.y - startPosition.y; // Calculate Py from diagram
            ...
            var displacementXZ = new Vector3(targetPosition.x - startPosition.x, 0, targetPosition.z - startPosition.z); // Calculate Px but for XZ axis not just X 
            var time = Mathf.Sqrt(-2 * _trajectoryHeight / _gravity) + Mathf.Sqrt(2 * (displacementY - _trajectoryHeight) / _gravity); // This is  Thorizontal  = Tup + Tdown
